...
public class BoardDao {
    ...
    public Map<String, Object> findById(int boardId) throws SQLException {
        String sql = "SELECT b.bid, b.name, b.pid, p.name AS parentName " +
                "FROM bankuai b LEFT JOIN bankuai p ON b.pid = p.bid WHERE b.bid = ?";
        try (Connection conn = DB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, boardId);
            try (ResultSet rs = ps.executeQuery()) {
                ...
                board.put("parentId", rs.wasNull() ? null : pid);
                board.put("parentName", rs.getString("parentName"));
                return board;
            }
        }
    }
    ...
    public List<Map<String, Object>> listLeafBoards() throws SQLException {
        String sql = "SELECT b.bid, b.name, b.pid, p.name AS parentName " +
                ...
                "WHERE b.pid IS NOT NULL ORDER BY p.bid, b.bid";
        List<Map<String, Object>> res = new ArrayList<>();
        try (Connection conn = DB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
